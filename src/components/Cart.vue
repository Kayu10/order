...
// 計算「至少 20 分鐘後」的最早可預約時間
const minCustomTime = computed(() => {
  return getTimeString(20)
})

// 切換下拉選單時同步更新到 Pinia Store
const handleTimeModeChange = () => {
  if (timeMode.value === 'ASAP') {
    cartStore.pickupTime = currentLang.value === 'en' ? 'ASAP' : '儘快製作'
  } else if (timeMode.value === '30m') {
    cartStore.pickupTime = currentLang.value === 'en' ? `In 30 mins (${getTimeString(30)})` : `30 分鐘後 (${getTimeString(30)})`
  } else if (timeMode.value === '45m') {
    cartStore.pickupTime = currentLang.value === 'en' ? `In 45 mins (${getTimeString(45)})` : `45 分鐘後 (${getTimeString(45)})`
  } else if (timeMode.value === '1h') {
    cartStore.pickupTime = currentLang.value === 'en' ? `In 1 hour (${getTimeString(60)})` : `1 小時後 (${getTimeString(60)})`
...
const validateCustomTime = () => {
  if (customTimeInput.value < minCustomTime.value) {
    const alertMsg = currentLang.value === 'en'
      ? `Time too close! Please choose a time at least 20 minutes from now (after ${minCustomTime.value}).`
      : `取餐時間過近！店家製作需要時間，請選擇至少 20 分鐘後（${minCustomTime.value} 以後）的時間。`
    alert(alertMsg)
    customTimeInput.value = minCustomTime.value
  }
  cartStore.pickupTime = currentLang.value === 'en' ? `Custom Time: ${customTimeInput.value}` : `自訂取餐時間 ${customTimeInput.value}`
...
const openConfirmModal = () => {
  ...
  if (cartStore.diningType === 'dine-in' && !cartStore.tableNumber?.trim()) {
    alert(currentLang.value === 'en' ? 'Please enter table number!' : '請輸入桌號！')
    return
  }

  // 檢查外帶電話
  if (cartStore.diningType === 'takeout' && !cartStore.contactPhone?.trim()) {
    alert(currentLang.value === 'en' ? 'Please enter contact phone number!' : '請輸入聯絡電話！')
    return
  }
  ...
  if (cartStore.diningType === 'delivery') {
    if (!cartStore.contactPhone?.trim()) {
      alert(currentLang.value === 'en' ? 'Please enter contact phone number for delivery!' : '請輸入外送聯絡電話！')
      return
    }
    if (!cartStore.deliveryAddress?.trim()) {
      alert(currentLang.value === 'en' ? 'Please enter delivery address!' : '請輸入外送地址！')
      return
    }
  }
...
<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 sticky top-20">
    ...
    <div class="flex justify-between items-center border-b pb-3 mb-4">
      ...
      >
        {{ currentLang === 'en' ? 'Clear Cart' : '清空購物車' }}
      </button>
    </div>
    ...
    <div class="mb-4 space-y-3 bg-gray-50 p-3 rounded-xl border border-gray-100">
      ...
      <div v-if="cartStore.diningType === 'dine-in'" class="space-y-1">
        <label class="block text-xs font-bold text-gray-700">
          {{ currentLang === 'en' ? 'Table No.' : '桌號' }} <span class="text-red-500">*</span>
        </label>
        <input 
          v-model.trim="cartStore.tableNumber"
          type="text" 
          :placeholder="currentLang === 'en' ? 'e.g. Table A01' : '例如：A01 桌'"
      ...
      <div v-else-if="cartStore.diningType === 'takeout'" class="space-y-1">
        <label class="block text-xs font-bold text-gray-700">
          {{ currentLang === 'en' ? 'Phone Number' : '聯絡電話' }} <span class="text-red-500">*</span>
        </label>
        <input 
      ...
      <div v-else-if="cartStore.diningType === 'delivery'" class="space-y-2">
        ...
        <div class="space-y-1">
          <label class="block text-xs font-bold text-gray-700">
            {{ currentLang === 'en' ? 'Delivery Address' : '外送地址' }} <span class="text-red-500">*</span>
          </label>
          <input 
            v-model.trim="cartStore.deliveryAddress"
            type="text" 
            :placeholder="currentLang === 'en' ? 'Enter delivery address' : '請輸入完整地址'"
            class="w-full text-xs p-2 border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
      </div>
      ...
      <div class="pt-2 border-t border-gray-200 space-y-2">
        <label class="block text-xs font-bold text-gray-700">
          <template v-if="currentLang === 'en'">
            {{ cartStore.diningType === 'delivery' ? 'Estimated Delivery Time' : 'Estimated Pickup Time' }}
          </template>
          <template v-else>
        ...
        >
          <option value="ASAP">
            {{ currentLang === 'en' ? '⚡ ASAP (As soon as possible)' : '⚡ 儘快製作（立即處理）' }}
          </option>
          <option value="30m">
            {{ currentLang === 'en' ? `🕒 In 30 mins (${getTimeString(30)})` : `🕒 30 分鐘後取餐 (${getTimeString(30)})` }}
          </option>
          <option value="45m">
            {{ currentLang === 'en' ? `🕒 In 45 mins (${getTimeString(45)})` : `🕒 45 分鐘後取餐 (${getTimeString(45)})` }}
          </option>
          <option value="1h">
            {{ currentLang === 'en' ? `🕒 In 1 hour (${getTimeString(60)})` : `🕒 1 小時後取餐 (${getTimeString(60)})` }}
          </option>
          <option value="custom">
            {{ currentLang === 'en' ? '✏️ Custom Time' : '✏️ 顧客自行填寫取餐時間' }}
          </option>
        </select>
        ...
        <div v-if="timeMode === 'custom'" class="space-y-1 pt-1">
          ...
          <p class="text-[10px] text-gray-400">
            {{ currentLang === 'en' ? `⚠️ Time must be at least 20 mins from now (Earliest: ${minCustomTime})` : `⚠️ 取餐時間至少需為 20 分鐘後（最早為 ${minCustomTime}）` }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="cartStore.cartItems.length === 0" class="text-center py-8 text-gray-400">
      <p class="text-sm">{{ currentLang === 'en' ? 'Your cart is empty' : '尚未選擇任何餐點' }}</p>
    </div>
    ...
    <div v-else class="space-y-4">
      <div class="space-y-3 max-h-[40vh] overflow-y-auto pr-1">
        ...
        >
          <div class="flex-1 pr-2">
            ...
            <p class="text-xs text-gray-400 mt-0.5">${{ item.price }} {{ currentLang === 'en' ? '/ portion' : '/ 份' }}</p>
          </div>
      ...
      <div class="pt-3 border-t border-gray-100 space-y-3">
        <div class="space-y-1">
          <label class="block text-xs font-bold text-gray-700 flex items-center gap-1">
            ...
            <span class="text-gray-400 font-normal">({{ currentLang === 'en' ? 'Optional' : '選填' }})</span>
          </label>
          <textarea 
            v-model="cartStore.orderNote"
            rows="2"
            :placeholder="currentLang === 'en' ? 'e.g. Extra sauce, no utensils...' : '例如：請提供環保餐具、醬汁分開放...'"
      ...
      <div class="pt-2 border-t border-gray-200 space-y-2">
        <div class="flex justify-between text-lg font-bold text-gray-900">
          <span>{{ currentLang === 'en' ? 'Total' : '總計金額' }}</span>
