    ...
    <div v-if="isOpen" class="modal-overlay" @click.self="handleClose">
      <div class="modal-content">
        ...
        <div class="text-center py-2">
          ...
          <h2 class="text-xl font-extrabold text-gray-800">訂單送出成功！</h2>
          <p class="text-xs text-gray-500 mt-1">店家已收到您的訂單，正在準備中</p>
        </div>
        ...
        <div class="bg-gray-50 border border-gray-100 rounded-xl p-4 my-4 space-y-3">
          ...
          <div class="flex justify-between items-center border-b border-gray-200 pb-2">
            <span class="text-xs text-gray-500 font-medium">訂單編號</span>
            <span class="text-sm font-bold font-mono text-blue-600">{{ orderData?.orderId }}</span>
          </div>

          <!-- 用餐方式與預計時間 -->
          <div class="flex justify-between items-center border-b border-gray-200 pb-2">
            <span class="text-xs text-gray-500 font-medium">預計取餐/送達</span>
          ...
          <div class="flex justify-between items-center text-xs">
            <span class="text-gray-500 font-medium">用餐資訊</span>
            <span class="font-bold text-gray-800">
              {{ diningTypeLabel }}
              <template v-if="orderData?.tableNumber"> (桌號: {{ orderData.tableNumber }})</template>
            </span>
          </div>
          ...
          <div v-if="orderData?.orderNote && orderData?.orderNote !== '無'" class="flex justify-between items-start text-xs border-t border-gray-200 pt-2">
            <span class="text-gray-500 font-medium shrink-0">訂單備註</span>
          ...
          <div v-if="orderData?.taxId && orderData?.taxId !== '無'" class="flex justify-between items-center text-xs border-t border-gray-200 pt-2">
            <span class="text-gray-500 font-medium">發票統編</span>
        ...
        <div class="text-center bg-blue-50/50 p-3 rounded-xl border border-blue-100 mb-4">
          <p class="text-xs text-gray-600 font-bold mb-2">請向店員出示此 QR Code 取餐</p>
          <img 
            :src="qrCodeUrl" 
            alt="取餐 QR Code" 
        ...
        <div class="border-t pt-3 mb-4">
          <details class="text-xs text-gray-600">
            <summary class="font-bold cursor-pointer text-gray-700 hover:text-blue-600 flex justify-between">
              <span>觀看餐點明細 (共 {{ orderData?.items?.length }} 項)</span>
              <span>小計：${{ orderData?.totalPrice }}</span>
            </summary>
...
const diningTypeLabel = computed(() => {
  ...
  return map[props.orderData?.diningType] || '🍽️ 內用'
})
